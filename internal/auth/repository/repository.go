...
func (r *repo) PasswordRecover(ctx context.Context, userID uint, password string) error {
	result := r.db.WithContext(ctx).Model(&models.User{}).Where("id = ?", userID).Update("password", password)
	if result.Error != nil {
		return result.Error
	}
	return nil
